...
class SolrIndex(EmbeddingIndex):
    ...
    async def _apply_chunk_window_expansion(
        ...
        Uses family_token_budget for chunks that have values for all configured
        chunk_family_fields, and orphan_token_budget for chunks missing any of
        those field values.
        ...
        async with self._create_http_client() as client:
            for chunk, score in zip(initial_response.chunks, initial_response.scores):
                ...
                token_budget = (
                    schema.orphan_token_budget if is_orphan
                ...
                if total_chunks < min_chunk_window or total_tokens <= schema.family_token_budget:
                    ...
                    window_tokens = sum(
                        c.get(schema.chunk_token_count_field, 0)
    ...
    def _expand_chunk_window(
        ...
        This algorithm starts with the matched chunk and expands up/down,
        ...
        while total_tokens < token_budget and (up > 0 or down < n):
            added = False

            # Try to add chunk up (earlier in document)
            if up > 0:
                next_chunk = chunks[up - 1]
                ...
                if total_tokens + next_tokens <= token_budget:
                    ...
                    added = True
                    log.info(
                        f"Added up chunk at index {up}, total_tokens={total_tokens}"
                    )

            # Try to add chunk down (later in document)
            if down < n:
                next_chunk = chunks[down]
                next_tokens = next_chunk.get(schema.chunk_token_count_field, 0)
                if total_tokens + next_tokens <= token_budget:
                    ...
                    down += 1
                    added = True
                    log.info(f"Added down chunk at index {down - 1}, total_tokens={
                            total_tokens
                        }")
    ...
    def _doc_to_chunk(self, doc: dict[str, Any]) -> Chunk | None:
        try:
            ...
            if self.chunk_window_config.chunk_token_count_field in doc:
                metadata[self.chunk_window_config.chunk_token_count_field] = doc[self.chunk_window_config.chunk_token_count_field]
